...
class BeamTraining:
    ...
    def set_up_models(self):
        ...
        self.f_c2 = Beampredict(self.f_input_size * 2, self.hidden_layers, self.opt.Batch_normal,  self.output_size).to(self.device).float()
    ...
    def test(self):
        ...
        self.optimizer = torch.optim.Adam(all_parameters,lr=self.learning_rate)
        self.optimizer.zero_grad()
        self.restore_model("./runs/Image/max_min_picture2/weights/best_beam.pt")
        self.validate()
    def validate(self):
        for model in self.models:
            model.eval()
