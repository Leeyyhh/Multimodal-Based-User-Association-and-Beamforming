...
from utils.beam_design_related import calculate_received_signal_strength_for_batches, beam_Logger, deduct_new_model_input2, construct_car_size
import numpy as np
import sys
...
class BeamTraining:
    ...
    def setup(self, training=True):
        ...
        self.nbs = 256 * 2  # nominal batch size
        ...
        self.new_feature=512
        self.feature_size = 512
        self.f_input_size = 512
    ...
    def set_up_models(self):
        ...
        self.feature_models = Beampredict(self.model_inputs_size, self.hidden_layers, self.opt.Batch_normal,  self.new_feature,self.dropout_prob).to(self.device).float()
        self.f_a0 = Beampredict( self.new_feature, self.hidden_layers, self.opt.Batch_normal,  self.output_size,self.dropout_prob).to(self.device).float()
        self.f_s0 = Beampredict( self.new_feature, self.hidden_layers, self.opt.Batch_normal,  self.output_size,self.dropout_prob).to(self.device).float()
        self.f_c0 = Beampredict( self.f_input_size * 2, self.hidden_layers, self.opt.Batch_normal,  self.output_size,self.dropout_prob).to(self.device).float()
        self.f_a1 = Beampredict( self.f_input_size, self.hidden_layers, self.opt.Batch_normal,  self.output_size,self.dropout_prob).to(self.device).float()
        self.f_s1 = Beampredict( self.f_input_size, self.hidden_layers, self.opt.Batch_normal,  self.output_size,self.dropout_prob).to(self.device).float()
        self.f_c1 = Beampredict( self.f_input_size * 2, self.hidden_layers, self.opt.Batch_normal,  self.output_size,self.dropout_prob).to(self.device).float()
        self.f_a2 = Beampredict( self.f_input_size, self.hidden_layers, self.opt.Batch_normal,  self.output_size,self.dropout_prob).to(self.device).float()
        self.f_s2 = Beampredict( self.f_input_size, self.hidden_layers, self.opt.Batch_normal,  self.output_size,self.dropout_prob).to(self.device).float()
        self.f_c2 = Beampredict( self.f_input_size * 2, self.hidden_layers, self.opt.Batch_normal,  self.output_size,self.dropout_prob).to(self.device).float()
    ...
    def train_epoch(self, epoch):
        ...
        for pbar_index, (imgs, targets, paths) in enumerate(pbar):
            ...
            remove_loc_tensor = remove_loc_tensor.permute(0, 2, 1, 3).to(self.device)
            feature = self.feature_models(remove_loc_tensor.reshape(self.train_batch * self.car_num, -1)).view(self.train_batch, self.car_num, -1)
    ...
    def test(self):
        ...
        self.optimizer = torch.optim.Adam(all_parameters,lr=self.learning_rate)
        self.optimizer.zero_grad()
        self.restore_model("./runs\\train_noise1_lr\max_min_picture5\weights\\best_beam.pt")
        self.validate()
    def validate(self):
        for model in self.models:
            model.eval()
        ...
        with torch.no_grad():  # Disable gradient calculation
            for imgs, targets, paths in self.valid_loader:
                ...
                imgs_new, remove_loc_tensor, detect_label_tensor = self.preprocess_data(imgs, targets)
                remove_loc_tensor = remove_loc_tensor.permute(0, 2, 1, 3).to(self.device)
                feature = self.feature_models(remove_loc_tensor.reshape(self.valid_batch * self.car_num, -1)).view(self.valid_batch, self.car_num, -1)
                fc0_output = self.aggregation_block(feature, self.f_s0, self.f_a0, self.f_c0)
                fc1_output = self.aggregation_block(fc0_output, self.f_s1, self.f_a1, self.f_c1)
        ...
        if new_loss2 < self.min_loss and self.training:
            ...
            ckpt = {
                "epoch": self.epoch,
                "models": {f'model_{i}': model.state_dict() for i, model in enumerate(self.models)},
...
if __name__ == '__main__':
    ...
    loss_function_name = "max_min_picture"
    training_label=1
    train_save_dir = create_directory("./runs/Image/", loss_function_name,training=training_label)
